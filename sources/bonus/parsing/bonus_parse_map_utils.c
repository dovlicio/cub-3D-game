...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bonus_parse_map_utils.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: vdamnjan <vdamnjan@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/01 17:55:34 by vdamnjan          #+#    #+#             */
/*   Updated: 2024/03/12 16:41:49 by vdamnjan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_row(t_parse *parse, char *str)
{
	...
	if (ft_strchr(parse->tmp, '1') || ft_strchr(parse->tmp, '0') || \
		ft_strchr(parse->tmp, '2') || ft_strchr(parse->tmp, '3'))
		start = 1;
	free(parse->tmp);
	parse->tmp = NULL;
	return (start);
...
void	ft_loop_data(t_parse *parse, char *str, int *i)
{
	if (!ft_strncmp(str, "NO ", 3))
		...
		ft_handle_textures(parse, str, i, 5);
	else if (!ft_strncmp(str, "F ", 2))
		ft_handle_textures(parse, str, i, 6);
	else
